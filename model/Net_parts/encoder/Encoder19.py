...
class Encoder19(nn.Module):
    def __init__(self, in_channels, base_channels=32, b2=UNetRDLEB2, use_bn=True, dropout_p=0.15):
        super().__init__()
        # 四层编码器
        self.enc1 = b2(in_channels, base_channels, use_bn=use_bn, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        if x5.shape[2:] != (32, 32):
            x5 = nn.functional.interpolate(x5, size=(32, 32), mode='bilinear', align_corners=False)
