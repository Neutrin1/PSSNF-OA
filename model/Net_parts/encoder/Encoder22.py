...
from PSSNF.model.Net_parts.public.CRH380AM import LSCEncoderBlock,CSGate,AttentiveGate
from PSSNF.model.Net_parts.public.UNetBlock import UNetEncoderBlock

class Encoder22(nn.Module):
    def __init__(self, in_channels, base_channels=64):
        super(Encoder22, self).__init__()
        ...
        # 注意力门控融合模块
        self.AttentionGatedFusion = AttentiveGate(base_channels)
        self.AttentionGatedFusion2 = AttentiveGate(base_channels*2)
        self.AttentionGatedFusion3 = AttentiveGate(base_channels*4)
        self.AttentionGatedFusion4 = AttentiveGate(base_channels*8)
        
        self.csgate1 = CSGate(base_channels)
        self.csgate2 = CSGate(base_channels*2)
        self.csgate3 = CSGate(base_channels*4)
        self.csgate4 = CSGate(base_channels*8)
    ...
    def forward(self, x):
        ...
        #CRH380B
        # 使用CSGate进行特征融合
        skip1 = self.csgate1(skip1, b1)
        skip2 = self.csgate2(skip2, b2)
        skip3 = self.csgate3(skip3, b3)
        skip4 = self.csgate4(skip4, b4)


        #CRH380C
        # 使用注意力门控进行特征融合
        # skip1 = self.AttentionGatedFusion(skip1, b1)
...
if __name__ == "__main__":
    ...
    # 创建模型实例
    model = Encoder22(in_channels=3, base_channels=64)
    model.eval()
