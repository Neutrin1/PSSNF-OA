...
from utils.metrics_utils import (
    ...
    TverskyLoss, FocalTverskyLoss, EnhancedComboLoss
)
...
def parse_args():
    ...
    parser.add_argument('--loss_type', type=str, default='combined', 
                        choices=['bce', 'dice', 'combined', 'tversky', 'focal_tversky','lovasz_hinge'],
...
def process_model_outputs_and_calculate_loss(model_outputs, masks, criterion):
    """
    🔥 HXD1C 模型输出处理函数
    处理 HXD1C 返回的字典格式输出并计算综合损失
    
    Args:
        model_outputs: 模型输出 (可能是字典、元组或张量)
        masks: 目标掩码
        criterion: 损失函数
        
    Returns:
        tuple: (用于计算指标的主输出, 总损失)
    """
    if isinstance(model_outputs, dict):
        # HXD1C 模型返回字典格式
        # {'logits': final_output, 'logits_main': main, 'logits_boundary': boundary, 'logits_center': center}
        
        logits_final = model_outputs["logits"]           # 最终输出（主要用于评估）
        logits_main = model_outputs["logits_main"]       # 主分支
        logits_boundary = model_outputs["logits_boundary"] # 边界分支  
        logits_center = model_outputs["logits_center"]   # 中线分支
        
        # 计算各分支损失
        final_loss = criterion(logits_final, masks)
        main_loss = criterion(logits_main, masks) 
        boundary_loss = criterion(logits_boundary, masks)
        center_loss = criterion(logits_center, masks)
        
        # 🔥 HXD1C 专用损失权重组合策略
        # 最终输出 + 主分支（高权重） + 辅助分支（适中权重）
        total_loss = (
            1.0 * final_loss +      # 最终输出损失（权重1.0）
            0.7 * main_loss +       # 主分支损失（权重0.7）
            0.4 * boundary_loss +   # 边界辅助损失（权重0.4）
            0.4 * center_loss       # 中线辅助损失（权重0.4）
        )
        
        # 返回最终输出用于指标计算
        outputs_for_metrics = logits_final
        ...
        if isinstance(model_outputs, list):
            outputs_for_metrics = model_outputs[0]
        else:
            outputs_for_metrics = model_outputs
        total_loss = criterion(outputs_for_metrics, masks)
    
    return outputs_for_metrics, total_loss


def process_model_outputs_for_visualization(model_outputs):
    """
    🔥 HXD1C 模型可视化输出处理函数
    从模型输出中提取用于可视化的张量
    ...
    Returns:
        torch.Tensor: 用于可视化的输出张量
    """
    if isinstance(model_outputs, dict):
        # HXD1C 模型: 使用最终输出进行可视化
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, 
    ...
    """Train segmentation model with HXD1C support, mixed precision and IoU-based early stopping"""
    ...
    for epoch in range(num_epochs):
        epoch_start_time = timer.start_epoch()
        ...
        for batch_idx, batch in enumerate(train_progress):
            ...
            if use_amp:
                with autocast():
                    ...
                    outputs, loss = process_model_outputs_and_calculate_loss(
                        model_outputs, masks, criterion
                    )
            else:
                model_outputs = model(images)
                outputs, loss = process_model_outputs_and_calculate_loss(
        ...
        if epoch % 30 == 0 or epoch == num_epochs - 1:
            ...
            with torch.no_grad():
                ...
                # 🔥 处理 HXD1C 输出用于可视化
                outputs = process_model_outputs_for_visualization(model_outputs)
...
def main():
    ...
    print("🔥 HXD1C model training with multi-branch supervision support...")
